...
**Purpose**: Remove all legacy artifacts that pollute the codebase, so that subsequent user story verification tasks operate against a clean baseline.
...
- [ ] T007 [US1] Verify that app/frontend/components/auth-form.tsx contains no third-party auth buttons (Google, Facebook, etc.) and renders only email, password, and password confirmation fields for the SignUp variant
...
- [ ] T019 [US3] Verify that app/frontend/components/authenticator.tsx reads accessToken from the Zustand store on mount and calls GET /api/v1/users/me to validate it
...
- [ ] T028 [US5] Verify that app/frontend/components/auth-form.tsx displays an inline error when the email field is left empty or contains an invalid format
- [ ] T029 [US5] Verify that app/frontend/components/auth-form.tsx displays an inline error when the password is shorter than 6 characters
...
- [ ] T032 Verify that unauthenticated users accessing /profile are handled appropriately — either redirected to /sign-in or shown an appropriate state via the Authenticator component in app/frontend/components/authenticator.tsx and the route at app/frontend/routes/profile.tsx
- [ ] T033 Verify that unauthenticated users accessing /admin are handled appropriately via the Authenticator component and the admin guard in app/frontend/routes/admin.tsx
